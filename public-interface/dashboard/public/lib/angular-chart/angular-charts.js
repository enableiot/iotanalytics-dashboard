/*
 Copyright (c) 2013 Chinmay - https://github.com/chinmaymk/angular-charts
 License: MIT
 */
      ...
      function pieChart() {
        ...
        if (!!config.labels) {
          path.append('text').attr('transform', function (d) {
            ...
            return d.data.label || d.data.y[0];
          });
        }
        function tweenPie(b) {
          b.innerRadius = 0;
...
angular.module("right", []).run(["$templateCache", function($templateCache) {
  $templateCache.put("right",
    ...
    "		<td><div ng-attr-style='background:{{l.color}}; height:15px;width:15px;'></div></td>\n" +
    "		<td style='display: block; text-align: left; padding-left: 2px' ng-bind='l.title'></td>\n" +
    "	</tr>\n" +
    "	</table>\n" +
    "</div>");
}]);
