...
app.factory('ngTableParams', ['$q', '$log', function ($q, $log) {
    ...
    var ngTableParams = function (baseParameters, baseSettings) {
         ...
         */
        this.generatePagesArray = function (currentPage, numPages) {
            var maxBlocks, maxPage, maxPivotPages, minPage, pages;
            maxBlocks = 11;
            pages = [];
        ...
        this.numPages = function () {
            ...
            return Math.ceil(self.total() / self.count());
        };

        this.isCurrentPageExists = function () {
            return self.page() <= self.numPages();
        };

        this.reloadPages = function () {
            var self = this;
            //after delete of device number of pages will change but page stays same, so we want to go to the last page
            if(!self.isCurrentPageExists()) {
                self.page(self.numPages())
            }
            settings.$scope.pages = self.generatePagesArray(self.page(), self.numPages());
        };
...
angular.module('ngTable').run(['$templateCache', function ($templateCache) {
	...
	$templateCache.put('ng-table/filters/text.html', '<input type="text" name="{{column.filterName}}" ng-model="params.filter()[name]" ng-if="filter==\'text\'" class="input-filter form-control" placeholder="filter"/>');
