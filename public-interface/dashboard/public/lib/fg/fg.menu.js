...
function Menu(caller, options){
	...
	this.parentChoose = function (item) {
        $('#menuSelection').val($(item).text());
        if(options.chooseItemCallback){
            options.chooseItemCallback(
                {
...
Menu.prototype.setPosition = function(widget, caller, options) { 
	...
	helper.css({ position: 'absolute', left: dims.refX, top: dims.refY, width: dims.refW, height: dims.refH, 'z-index': 1000 });
	el.wrap(helper);
